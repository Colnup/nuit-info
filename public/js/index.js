...
function sendMessage(json) {
  ...
  if (json["by"] == "narrator") {
    ...
    content_text.textContent = json["message"]
      ? json["message"]
      : "[ERREUR DANS LE MESSAGE | VERIFIER LE MESSAGE DANS LES DONNEES!]";
  } else {
    // C'est un personnage
    message_image.src = "../data/img/avatars/" + json["by"] + ".png";
...
function setChoice(json) {
  ...
  left_clone.addEventListener("click", (ev) => {
    choice(json, "1");
  });

  right_clone.innerText = json["choix"]["2"]["message"];
  right_clone.addEventListener("click", (ev) => {
    choice(json, "2");
  });
